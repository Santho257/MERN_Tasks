...
const EditExpense = () => {
    ...
    const [formdata, setFormdata, errors, setErrors] = useFetch(
        `${BASE_URL}/expenses/${expId}`,
    ...
    const deleteExpense = useCallback(async (e) => {
        try {
            const result = await axios.delete(
                `${BASE_URL}/expenses/${expId}`,
                { headers: { Authorization: user.token } }
            );
            if (result.data.success) navi(-1);
        } catch (error) {
        ...
        <Section className={styles.container}>
            <Section>
                <Form onSubmit={entryExpense}>
                    <Title>
                        <span onClick={() => navi(-1)}><Back /></span>
                        Edit Expense
                        <span onClick={deleteExpense}><Delete /></span>
                    </Title>
                    <Input
                        type="text"
                        name="source"
